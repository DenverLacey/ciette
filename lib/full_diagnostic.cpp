#include "full_diagnostic.h"

#include <math.h>
#include <iomanip>

namespace ciette {

    std::ostream& FullDiagnostic::write(std::ostream& s) const {
        auto snippet = this->span.snippet(this->source);
        int width = log10(snippet.lineno + 1) + 1;

        s << "--- " << this->kind << " ------------" << std::endl;

        s << std::setw(width) << ' ' << " |" << std::setw(0) << std::endl;
        s << snippet.lineno << " |" << snippet.line << std::endl;
        s << std::setw(width) << ' ' << " |" << std::setw(snippet.column) << ' ' << std::setw(snippet.length + 1) << std::setfill('^') << ' ' << std::setw(0) << std::setfill(' ') << std::endl;
        s << std::setw(width) << ' ' << " |" << std::setw(snippet.column) << ' ' << this->span.label() << std::setw(0) << std::endl;

        if (!this->notes.empty()) {
            for (const Note& note : this->notes) {
                switch (note.kind) {
                    case NoteKind::Note:
                        s << "Note: ";
                        break;
                    case NoteKind::Hint:
                        s << "Hint: ";
                        break;

                    default:
                        assert(!"Invalid NoteKind");
                }

                s << note.note << std::endl;
            }
        }

        return s;
    }

}
